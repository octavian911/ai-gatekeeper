...
note "2) Patch index.ts safely as TEXT (do NOT execute TS)"
# IMPORTANT: use `node -` so node runs the stdin script, and we pass INDEX_TS as argv[2]
node - "$INDEX_TS" <<'NODE'
const fs = require("fs");

const file = process.argv[2]; // argv[0]=node, argv[1]='-'
...
`;
  ...
  // Insert right after stripper block if present, else after app creation
...
note "DONE ✅"
echo "Key check:"
echo "  - /api/__health should be JSON 200."
echo "  - If /api/__health is 200 but baselines are 404 => backend routes differ; we will map UploadPanel to the real routes."
